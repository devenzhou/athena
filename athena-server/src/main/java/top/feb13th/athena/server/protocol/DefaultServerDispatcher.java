...
import top.feb13th.athena.core.exception.Assertion;
import top.feb13th.athena.core.exception.AssertionError;
...
import top.feb13th.athena.core.message.ResponseGenerator;
import top.feb13th.athena.server.bean.ServerModuleBeanHolder;
import top.feb13th.athena.server.bean.ServerModuleBeanWrapper;
import top.feb13th.athena.server.session.DefaultSession;
import top.feb13th.athena.server.session.Session;
import top.feb13th.athena.server.session.SessionHolder;
import top.feb13th.athena.server.support.ConnectionChecker;
import top.feb13th.athena.core.support.SystemStatusCode;
...
public class DefaultServerDispatcher extends MessageToMessageDecoder<Request> {
  ...
  protected void decode(ChannelHandlerContext ctx, Request msg, List<Object> out) throws Exception {
    ...
    ServerModuleBeanWrapper wrapper = ServerModuleBeanHolder.get(module, command);
    if (wrapper == null) {
      // 资源找不到
      if (logger.isDebugEnabled()) {
   ...
   */
  private void invokeMethod(ServerModuleBeanWrapper wrapper, Channel channel, Request request,
